<?php
class Hexa_Author_Information extends WP_Widget
{
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            ...
            'Hexa Author', // Name
            array('description' => esc_html__('A Author Widget', 'hexa-theme'), 'classname' => 'hexa_author-widget') // Args
        );
    }
    ...
    public function author_scripts()
    {
        ...
        wp_enqueue_media();
        wp_enqueue_script('hexa_upload_media_admin', get_template_directory_uri() . '/inc/assets/js/upload_media_widget.js', array('jquery'));
    }
...
?>
        <div class="author-widget text-center">
            <div class="author-widget-image mb-20">
                <img src="<?php echo esc_url($image); ?>" alt="<?php echo esc_attr($title); ?>">
            </div>
            <div class="author-widget-info">
                <?php
                if (!empty($title)) {
                    echo '<h5 class="author-widget-title mb-10">' . $title . '</h5>';
                }
                if (!empty($text)) {
                    echo '<p class="author-widget-text mb-20">' . $text . '</p>';
                }
                ?>
                <div class="author-widget-social">
                    <?php if ($facebook) {
                        echo '<a class="social-facebook" href="' . esc_url($facebook) . '"><i class="fab fa-facebook-f"></i></a>';
                    } ?>
                    <?php if ($linkedin) {
                        echo '<a class="social-linkedin" href="' . esc_url($linkedin) . '"><i class="fab fa-linkedin-in"></i></a>';
                    } ?>
                    <?php if ($instagram) {
                        echo '<a class="social-instagram" href="' . esc_url($instagram) . '"><i class="fab fa-instagram"></i></a>';
                    } ?>
                    <?php if ($twitter) {
                        echo '<a class="social-twitter" href="' . esc_url($twitter) . '"><i class="fab fa-twitter"></i></a>';
                    } ?>
                    <?php if ($youtube) {
                        echo '<a class="social-youtube" href="' . esc_url($youtube) . '"><i class="fab fa-youtube"></i></a>';
                    } ?>
                    <?php if ($pinterest) {
                        echo '<a class="social-pinterest" href="' . esc_url($pinterest) . '"><i class="fab fa-pinterest-p"></i></a>';
                    } ?>
                    <?php if ($dribbble) {
                        echo '<a class="social-dribbble" href="' . esc_url($dribbble) . '"><i class="fab fa-dribbble"></i></a>';
                    } ?>
                    <?php if ($google) {
                        echo '<a class="social-google" href="' . esc_url($google) . '"><i class="fab fa-google-plus-g"></i></a>';
                    } ?>
                </div>
            </div>
        </div>
    ...
    ?>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php echo esc_html__('Name:', 'hexa-theme'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>">
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('text')); ?>"><?php echo esc_html__('Description:', 'hexa-theme'); ?></label>
...
add_action('widgets_init', 'hexa_author_register_widgets');
function hexa_author_register_widgets()
{
    register_widget('Hexa_Author_Information');
}
